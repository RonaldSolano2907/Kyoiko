...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "BEGIN PaqueteDepartamento.ActualizarDepartamento(:id, :cedula_jefe, :nombre, :descripcion); END;";
    ...
    if (oci_execute($stmt)) {
        ...
        echo "Error al actualizar el departamento: " . htmlentities($error['message']);
    }
    oci_free_statement($stmt);
}

// Leer los datos actuales del departamento
$sql = "BEGIN PaqueteDepartamento.LeerDepartamento(:id, :cursor); END;";
$stmt = oci_parse($conn, $sql);
...
<body>
    ...
    <div class="form-container">
        ...
        <form action="edit.php?id=<?= $id ?>" method="POST">
            ...
            <input type="text" id="cedula_jefe" name="cedula_jefe" value="<?= htmlspecialchars($data['CEDULAJEFEDEPARTAMENTO']) ?>" required>

            <label for="nombre">Nombre</label>
            <input type="text" id="nombre" name="nombre" value="<?= htmlspecialchars($data['NOMBRE']) ?>" required>

            <label for="descripcion">Descripción</label>
            <textarea id="descripcion" name="descripcion" required><?= htmlspecialchars($data['DESCRIPCION']) ?></textarea>
